...
processor stack (tokenizer with the media special-token attributes and a patched chat template that also
accepts the standard list-of-content-blocks message format, image processor, audio feature extractor, unified
processor config). `--processor_only` rewrites just the processor stack without re-sharding the weights.
...
# The upstream chat template accepts user content only as a string or a `{"parts": [...]}` mapping. This branch
# additionally accepts the standard Transformers list-of-content-blocks format so `apply_chat_template` works
# out of the box; it renders identically to the "parts" form. To be upstreamed to apertus-omni-tokenizer.
_TEMPLATE_PATCH_ANCHOR = """                {%- endfor -%}
            {%- else -%}
                {{- raise_exception("Invalid user message: " + message.role) -}}"""
_TEMPLATE_LIST_CONTENT_BRANCH = """                {%- endfor -%}
...
def patch_chat_template(chat_template: str) -> str:
    """Adds a list-of-content-blocks branch to the user-message rendering (see note above)."""
    if "message.content is not string and message.content is not mapping" in chat_template:
        return chat_template  # already patched
    if chat_template.count(_TEMPLATE_PATCH_ANCHOR) != 1:
        raise ValueError(
            "The source chat template does not match the expected Apertus 1.5 layout; cannot add the "
            "list-of-content-blocks branch. Update the patch anchor in this script."
        )
    return chat_template.replace(_TEMPLATE_PATCH_ANCHOR, _TEMPLATE_LIST_CONTENT_BRANCH)


def build_processor(apertus_checkpoint: str, audio_tokenizer_config: dict) -> Apertus1p5Processor:
    tokenizer = AutoTokenizer.from_pretrained(apertus_checkpoint, extra_special_tokens=EXTRA_SPECIAL_TOKENS)
    # batched generation requires left padding; pin it rather than relying on the backbone artifact
...
def verify(composite_dir: str, max_new_tokens: int = 12):
    ...
    token_id_pairs = [
        ...
        (processor.boi_token, config.boi_token_id),
        (processor.eoi_token, config.eoi_token_id),
        (processor.image_wrapper_token, config.image_wrapper_token_id),
        (processor.eol_token, config.eol_token_id),
        (processor.audio_start_token, config.audio_start_token_id),
        (processor.audio_end_token, config.audio_end_token_id),
    ]
    ...
    if not (processor_ok and token_ids_ok):
        failed_checks.append("processor")
    print(
        f"[{'PASS' if processor_ok and token_ids_ok else 'FAIL'}] processor: components {component_classes}, "
        f"all 8 structure-token ids match the config: {token_ids_ok}"
    )
...
def write_processor(apertus_checkpoint: str, audio_tokenizer_checkpoint: str, output_dir: str):
    """Writes the processor stack (tokenizer incl. special-token attributes and patched chat template, image
